...
def extract_data(data_file_path, date_column_name="date", date_format="%d-%m-%Y"):
    ...
    df[date_column_name] = pd.to_datetime(
        df[date_column_name], format=date_format)
...
df_hosp_melt = df_hosp.melt(id_vars=['date', 'province'], value_vars=[
                            'currently_admitted', 'current_in_icu'])
df_hosp_melt.columns = ['date', 'variable', 'type', 'value']

df_daily_cases = make_covid19za_data_long(CASES_PATH, type='daily_cases')
df_cummulative_cases = make_covid19za_data_long(
    CASES_PATH, type='cummulative_cases')
...
df_cummulative_deaths = make_covid19za_data_long(
    DEATHS_PATH, type='cummulative_deaths')
df_average_deaths = make_covid19za_data_long(
    DEATHS_PATH, type='average_deaths')


df = concat_long_covid19_za_data([df_daily_cases, df_daily_deaths, df_cummulative_cases,
                                 df_cummulative_deaths, df_average_cases, df_average_deaths])
